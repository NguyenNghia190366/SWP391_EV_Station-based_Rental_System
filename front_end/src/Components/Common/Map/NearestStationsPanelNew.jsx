...
const NearestStationsPanelNew = ({ onFindNearest, nearestStation, nearestSearching }) => {
  ...
  const handleUseMyLocation = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Cannot get your location: ' + (err?.message || err));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFindByCoords = async (e) => {
    ...
    if (Number.isNaN(parsedLat) || Number.isNaN(parsedLng)) return alert('Please enter valid coordinates');
    setLoading(true);
    try {
      await onFindNearest?.({ lat: parsedLat, lng: parsedLng });
    } catch (err) {
      console.error(err);
      alert('Failed to find nearest station');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-sm bg-white text-gray-900 p-4 border border-gray-200 rounded-xl shadow-lg box-border">
      <h4 className="text-base font-bold text-gray-900 mb-3">️ Find the nearest rental station</h4>
      ...
      {nearestStation ? (
        <div className="mt-2 p-3 rounded-2xl bg-gradient-to-b from-white to-blue-50 border border-gray-100 shadow-sm">
          <strong className="block mb-1.5 text-base">{nearestStation.station?.station_name || nearestStation.station?.name || 'Station'}</strong>
          <div className="text-sm text-gray-500 mb-1.5">Distance: {(nearestStation.distanceKm ?? 0).toFixed ? (nearestStation.distanceKm).toFixed(2) + ' km' : `${nearestStation.distanceKm} km`}</div>
          {nearestStation.station?.address && <div className="text-sm text-gray-500">{nearestStation.station.address}</div>}
        </div>
      ) : (
        <div className="mt-2 text-sm text-gray-500">No results yet. Try using your location or enter coordinates.</div>
      )}
    </div>
  );
};
